...
train.py -> fit.py, RuntimeError: 0D or 1D target tensor expected, multi-target not supported, when the cross_entropy_loss receives the labels it received an unnessecary dimension of (,1) which does not fit the outputs that do not have this dimension, used the torch.squeeze method with labels.squeeze() in line 24,